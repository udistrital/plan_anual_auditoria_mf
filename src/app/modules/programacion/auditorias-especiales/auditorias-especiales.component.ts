...
export class AuditoriasEspecialesComponent implements OnInit {
  ...
  NewAuditoria() {
    ...
    this.planAnualAuditoriaService.post('/auditoria', {
      vigencia_id: this.selectedYearId,
    }).subscribe(
      (response: any) => {
        if (response.Status === 201) {
          this.modalService.mostrarModal('Auditoría especial creada exitosamente', 'success', 'AUDITORIA CREADA');
          this.loadAuditoriasFromService();
        }
      },
      (error) => {
        if (error.error?.Data && error.error.Data.includes('Ya existe una auditoría')) {
          ...
          this.modalService.mostrarModal('Error al crear la auditoría', 'error', 'ERROR');
        }
      }
    );
  } else {
